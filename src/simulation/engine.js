...
export class SimulationEngine {
    ...
    addNode(data) {
        ...
        // Ensure coordinates are numbers and clamped
        const safeX = parseFloat(data.x);
        const safeY = parseFloat(data.y);
        
        this.nodes.set(id, {
            id,
            x: !isNaN(safeX) ? safeX : 0, 
            y: !isNaN(safeY) ? safeY : 0,
            z: Number(data.z) || 0, // Add Z coordinate for 3D
