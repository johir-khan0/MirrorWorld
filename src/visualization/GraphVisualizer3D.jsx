...
function Node3D({ node, isSelected, onSelect, onMove }) {
    ...
    const position = useMemo(() => {
        ...
        // Adjust Y so the object sits on the "ground" (or moved up by elevation)
        // Base mesh origin is usually center, so we move up by half height
        // NodeGeometry base height is approx 1. So if sy=heightScale, actual height is heightScale.
        // We need to move up by heightScale/2.
        
        let visualHeight = 1;
        if ([NODE_TYPES.HOSPITAL, NODE_TYPES.WAREHOUSE, NODE_TYPES.ZONE, NODE_TYPES.HUB, NODE_TYPES.SHELTER].includes(node.type)) {
             visualHeight = Math.max(1, heightScale);
        }

        const y = (visualHeight * 0.5) + elevation; 
        
        return [x, y, zIndex];
    }, [node.x, node.y, node.z, heightScale, node.type]);
